...
func main() {
	...
	flags.IntVar(&opts.inCluster, "inCluster", 0, "should be assume the test is running in cluster. 0: endpoints, 1: ingress, 2: loadBalancer")
	eventLoadCmd.Execute()
}

// runLoadFlyWheel will actually run the test.
func runLoadFlyWheel() {
	err := validateOptions()
	if err != nil {
		fmt.Println(err)
		os.Exit(1)
	...
	clientset, err := util.InitClientSet(opts.kubeConfig)
	if err != nil {
		fmt.Println(err)
		os.Exit(1)
	}
	err = util.VerifyNamespaces(clientset)
	if err != nil {
		fmt.Println(err)
		os.Exit(1)
	}
	...
	if len(opts.killList) > 0 && opts.killInterval > 0 {
		go chaos.KillServices(stop, opts.killInterval, opts.killList)
	} else {
		fmt.Printf("Skipping the chaos killer function! (%d,%t,%t)\r\n", opts.inCluster, len(opts.killList) > 0, opts.killInterval > 0)
	}
...
func verifyPods(clientset *kubernetes.Clientset, namespace string, expected int) (bool, error) {
	result := false
	var reqErr error
	wait.Poll(250*time.Millisecond, 60*time.Second, func() (bool, error) {
		count := 0
...
func getExternalIP(clientset *kubernetes.Clientset, namespace string, name string, service string) (string, error) {
	...
	if opts.inCluster == 1 {
		...
		if len(endpoints.Subsets) != 1 {
			return "", errors.New(fmt.Sprintf("got %d subsets on endpoints %s:%s", len(endpoints.Subsets), namespace, service))
		}
		index, err := rand.Int(rand.Reader, big.NewInt(int64(len(endpoints.Subsets[0].Addresses))))
		if err != nil {
			return "", err
		}
		ip := endpoints.Subsets[0].Addresses[int(index.Int64())].IP
		port := endpoints.Subsets[0].Ports[0].Port
		return fmt.Sprintf("%s:%d", ip, port), nil
	}
	// opts.inCluster == 2 loadbalancer
	fmt.Printf("getexternalip for loadbalancer\n")
	result := ""
	var reqErr error
	wait.Poll(250*time.Millisecond, 30*time.Second, func() (bool, error) {
		...
		lb := status.LoadBalancer
		in := lb.Ingress
		//fmt.Printf("WJ len(in) is %s\n", len(in))
		if len(in) != 1 {
			//fmt.Printf("WJ len(in) is %d\n",len(in))
			//reqErr = errors.New(fmt.Sprintf("got %d addresses on loadbalancer", len(in)))
			return false, nil
		}
		result = in[0].IP
		fmt.Printf("WJ IP %s\n", result)
